...
flowchart TB
    S1["`**S1**
    Proyecto backend, BD, REST y OpenAPI`"]
    S2["`**S2**
    CRUD maestro, validaciones y pruebas`"]
    S3["`**S3**
    Objetos relacionados Categoria-Producto`"]
    S4["`**S4**
    Cabecera-detalle y transacciones`"]
    S5["`**S5**
    Consultas, reportes y CORS`"]
    S6["`**S6**
    Producto U1`"]
