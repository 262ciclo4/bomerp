...
- **Modelar y exponer** una asociación `@ManyToOne` entre dos entidades JPA mediante un DTO relacionado, validando referencias inexistentes, evitando ciclos de serialización, y habilitando una navegación controlada entre los dos recursos — completando además el CRUD de `Categoria`.
...
#### 1.6.1 Caso: la columna que nadie usó (`ID_CATEGORIA`)

Desde S1, el script de BD2 que crea las tablas del catálogo (`S01_02_tablas.sql`) define `PRODUCTOS` así:
...
La columna `ID_CATEGORIA` y su llave foránea existen en Oracle desde antes de que `Producto` (la entidad Java) tuviera un solo campo. Pero durante S1 y S2, la clase `Producto.java` solo mapeó `id`, `nombre`, `precio` y `stock` — nunca `ID_CATEGORIA`. `ddl-auto: validate` (S1, `application-dev.yml`) explica por qué eso nunca falló: ese modo solo valida que las columnas **mapeadas** existan, no exige que la entidad mapee **todas** las columnas de la tabla. La relación estuvo ahí, disponible, invisible para el backend, durante dos sesiones completas.

**Preguntas de análisis**

**Activación de conocimientos previos**

1. Si la columna y la llave foránea ya existían en Oracle, ¿por qué hacía falta escribir código Java nuevo para "usarlas"?
2. `ddl-auto: validate` no crea columnas — ¿qué hubiera pasado si `ID_CATEGORIA` **no** hubiera existido todavía en Oracle y esta sesión intentara mapearla igual?

**Comprensión de asociaciones ORM**

1. ¿Por qué `ProductoResponse` no debería incluir un `Categoria` completo (con su `descripcion`), sino un DTO más chico?
2. ¿Qué error concreto aparece si `categoriaId` en `ProductoRequest` apunta a una categoría que no existe, y quién lo debe capturar?
...
Un problema clásico de JPA: si `Categoria` tuviera un `@OneToMany` de vuelta hacia `List<Producto>`, y esas entidades se serializaran directamente a JSON, cada `Producto` intentaría serializar su `Categoria`, que intentaría serializar su lista de `Producto`, indefinidamente — `StackOverflowError`.

Esta sesión nunca llega a ese problema, por dos decisiones tomadas antes de que pudiera aparecer:
...
- **3.3** Crear `CategoriaMapper`.
- **3.4** Completar `CategoriaService`/`CategoriaServiceImpl`.
- **3.5** Completar `CategoriaController`.
- **3.6** Asociar `Producto` con `Categoria` en la entidad.
- **3.7** Actualizar los DTO de `Producto`.
- **3.8** Actualizar `ProductoMapper`.
- **3.9** Actualizar `ProductoServiceImpl` y `ProductoController`.
- **3.10** Probar la asociación completa.
- **3.11** Escribir pruebas automatizadas.
...
**Producto del paso:** `Producto.categoria`, mapeada a la columna `ID_CATEGORIA` que ya existe en Oracle (1.6.1).
