...
- Poner `@Entity` en la clase del dominio y llamarlo hexagonal (Tipo Mamani, ver Bibliografía) — una sola clase para negocio y persistencia sigue atada a JPA, aunque viva en un paquete llamado `dominio`; la separación real exige dos clases (una de dominio, una de persistencia) y un mapper entre ellas.
- Llamar "caso de uso" a un método que solo hace `repository.save(entidad)` — sin ninguna regla de negocio real de por medio, es un CRUD disfrazado de vocabulario hexagonal, no una indirección que proteja algo.
...
**La prueba del papel y lápiz, para no confundir Entidad con Caso de uso** (Tipo Mamani, ver Bibliografía): si la regla seguiría siendo cierta en una oficina con papel y lapicero, sin que existiera ningún software, es una **entidad** (una regla del negocio en sí misma). Si la regla solo tiene sentido porque existe el sistema — "registrar un pago vía API", por ejemplo — es un **caso de uso** (una intención concreta de la aplicación). La primera cambia rara vez, y se reutilizaría en cualquier sistema que automatizara el mismo negocio; la segunda cambia con el procedimiento, no con la regla.
...
17. Tipo Mamani, N. W. (2026). *Clean Architecture y Arquitectura Hexagonal con Java y Spring Boot* [material de curso: guía teórica y guía práctica]. Escuela Profesional de Ingeniería de Sistemas, UPeU.
